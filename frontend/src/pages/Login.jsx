...
export default function Login() {
    ...
    if (window.google?.accounts?.id) {
      window.google.accounts.id.prompt((notification) => {
        if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
          // One-tap was blocked/skipped — fall back to button-based popup
          window.google.accounts.id.renderButton(
            document.createElement('div'),
            { type: 'standard' },
          );
          // Use the popup approach instead
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-6 bg-cream">
      ...
      {/* Auth buttons */}
      <div className="w-full max-w-xs lg:max-w-sm flex flex-col gap-3">
        {/* OAuth */}
        <button
          onClick={handleGoogleLogin}
          className="flex items-center justify-center gap-3 w-full py-3 px-4 rounded-full border border-stone bg-warm-white font-semibold text-sm hover:border-terracotta transition-vouch"
        >
          ...
          {mode === 'register' && (
            <>
              <input
                ...
                required
                className="w-full px-4 py-3 rounded-full border border-stone bg-warm-white text-sm focus:outline-none focus:border-terracotta transition-vouch"
              />
              <input
                type="text"
      ...
      {/* Footer */}
      <p className="mt-12 text-[10px] text-secondary-text/50">
